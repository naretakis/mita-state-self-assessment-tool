...
export const StorageService = {
  ...
  saveAssessment: async (assessment: Assessment): Promise<boolean> => {
    try {
      ...
      try {
        localStorage.setItem(`${STORAGE_KEY_PREFIX}${assessment.id}`, JSON.stringify(assessment));
        ...
        console.warn(
          'localStorage save failed, would use IndexedDB in production',
          localStorageError
        );
        return false;
      }
